# 경험 블록 빌더 (Experience Block Builder) — 서비스 기획서

**프로젝트명**: 경험 블록 빌더  
**작성일**: 2026-04-06  
**상태**: 기획 완료 → 구현 시작  

---

## 1. 서비스 개요

### 핵심 컨셉
AI 대화형 인터뷰를 통해 사용자의 막연한 경험을 **구조화된 STAR 블록**으로 변환하는 서비스.

### 핵심 가치
- "빈 칸을 채워라"가 아니라 **"편하게 말해라"**
- 사용자가 모르는 자기 경험의 가치를 **AI가 발견하고 알려준다**
- 한번 만든 블록은 어떤 자소서/면접에든 **재사용 가능한 자산**

### 타겟 사용자
- 취업 준비생 (대학생 3~4학년, 졸업 후 1~2년)
- 이직 준비자 (경력 1~5년)
- 자기소개서/면접 준비에 어려움을 느끼는 사람

---

## 2. 경쟁 분석 및 차별점

### 기존 도구의 한계

| 기존 방식 | 문제점 |
|-----------|--------|
| ChatGPT에 "자소서 써줘" | 본인 경험이 아닌 AI 소설이 나옴 |
| 자소서 템플릿 사이트 | 빈 칸 앞에서 막힘, 채우기 어려움 |
| 취업 멘토링 | 비용 높고 (회당 5~10만원), 확장 불가 |
| LinkedIn + AI | 회사 매칭은 되지만 경험 구조화는 안 됨 |

### 우리의 차별점
1. **대화형 인터뷰**: 빈 칸이 아닌 자연스러운 대화로 경험 추출
2. **가치 발견**: AI가 사용자도 몰랐던 경험의 강점을 알려줌
3. **구조화된 자산**: 한번 만들면 반복 활용 가능한 경험 블록
4. **역량 커버리지**: 어떤 역량이 부족한지 시각화

---

## 3. 수익 모델

> **구현됨 (2026-06)**: value-first 게이팅 + 마감 패스(단건) + Pro 구독. 목표는 **구독 전환 극대화**.
> 상세는 [README — 결제/유료화](./README.md#결제--유료화). 아래 AI 비용표는 Gemini Flash 전환 이전(Haiku/Sonnet 기준) 추정으로 **현재는 superseded**.

### 플랜 (Edge Function 강제)

| 구분 | 무료 | 마감 패스 (₩4,900, 단건) | Pro (₩9,900/월, 첫 달 ₩4,900) |
|------|------|--------------------------|-------------------------------|
| 블록 생성 | 월 3개 | — | 무제한 |
| 자소서 (공고당) | **첫 문항만** | 이 공고 전 문항 | 모든 공고 전 문항 |
| 문항당 다시쓰기 | 3회 | 3회 | 3회 |

- **value-first**: 첫 문항 완성본을 무료로 보여준 뒤, 품질 확인 직후 나머지 문항에서 결제 유도(`402 locked` → 업그레이드 시트). 단건은 구독으로 미는 앵커(첫 달을 단건과 동가로).
- AI 는 **Gemini Flash** 1종(서버 프록시). 모델/토큰 서버 캡 + 문항당 3회 상한으로 비용 유계.

### (참고) 구버전 AI 비용 추정 — Haiku/Sonnet 기준, 현재 미적용

| 항목 | Haiku 3.5 | Sonnet 4 | 하이브리드 |
|------|-----------|----------|-----------|
| 1회 인터뷰 비용 | ~36원 | ~137원 | ~60원 |
| 1,000명 MAU 월간 | ~108,000원 | ~411,000원 | ~180,000원 |

---

## 4. 사용자 플로우

```
[가입/로그인]
     ↓
[메인 대시보드] — 내 경험 블록 목록 + 역량 커버리지
     ↓
[+ 새 블록 만들기]
     ↓
[카테고리 선택] (UI, AI 호출 없음)
  🎓 팀 프로젝트    💼 인턴/직무 경험
  🏆 공모전/대회    🔧 개인 프로젝트
  👥 동아리/대외활동  💪 아르바이트
  📚 자격증/교육    🌏 해외 경험
     ↓
[AI 대화형 인터뷰] (3~5분, 5턴 고정)
  턴 1: 워밍업 — 경험 윤곽 + 기억나는 장면
  턴 2: 상황 파기 — 맥락/규모/제약 조건
  턴 3: 행동 파기 — 구체적 행동 추출 (핵심 턴)
  턴 4: 결과/배움 — 결과 + 배운점 확인
  턴 5: 블록 생성 — 강제 생성 (정보 부족해도 최선의 블록)
     ↓
[블록 확인/수정] — 생성된 블록 검토 후 저장
     ↓
[블록 결과 화면] — 성취감 + 업종/직무 AI 추천 + 역량 현황
     ↓
[자소서 쓰기] or [블록 더 만들기]
     ↓ (자소서 쓰기 선택 시)
[공고 붙여넣기] — 채용 공고 텍스트 통째로 입력
     ↓
[AI 분석] — 회사/문항/JD/글자수 자동 파싱 + 블록↔문항 매칭
     ↓
[자소서 초안 생성] — 문항별 AI 자소서 생성, 인라인 편집, 글자수 표시
     ↓
[전체 복사 / 내보내기]
```

### 순환 구조

```
              ┌─── 블록 부족한 문항 발견 ───┐
              ↓                           │
  인터뷰 → 블록 생성 → 공고 입력 → 자소서 생성
              ↑                           │
              └──── 다른 회사 지원 ─────────┘
```

---

## 5. AI 대화 설계

### 인터뷰 턴 구조 (5턴 고정)

| 턴 | 목표 | AI 전략 |
|----|------|---------|
| 1. 워밍업 | 경험 윤곽 + 기억나는 장면 | 부담 낮은 질문, 상황+행동 단서 함께 유도 |
| 2. 상황 파기 | 맥락/규모/제약 구체화 | "제일 기억에 남는 순간?" "어떤 상황이었어요?" |
| 3. 행동 파기 | 구체적 행동 추출 (핵심) | "그래서 어떻게 했어요?" + 선택지 제시 |
| 4. 결과/배움 | 결과 + 배운점 보충 | "숫자로 표현하면?" "전후 비교하면?" |
| 5. 블록 생성 | 강제 구조화 | 정보 부족해도 최선의 블록 생성 (예외 없음) |

- 턴 4에서 시스템이 "다음이 마지막 턴" 힌트를 AI에 주입
- 턴 5에서 블록 미생성 시 자동 재요청으로 강제 생성

### 핵심 설계 원칙

1. **질문은 턴당 1~2개만**: 질문 폭탄 금지
2. **공감 먼저, 질문 나중**: "당황스러웠겠네요" → "그래서 어떻게 했어요?"
3. **되받아치기**: 사용자 답변 키워드를 반복하며 관심 표현
4. **"별거 없었어요" 대응**: "이건 좀 아닌데 싶었던 적 있어요?" 로 리다이렉트
5. **가치 리프레이밍**: "3명이서 B+이면 잘한 거죠"

### 분기 처리

```
사용자가 짧게 답변할 때:
  → "조금만 더 구체적으로 말해주시면 좋은 블록이 될 것 같아요"
  → 선택지 제시: "혹시 이런 상황이었나요? A) ... B) ..."

사용자가 여러 경험을 섞을 때:
  → "두 가지 다 좋은 소재인데, 하나씩 정리하면 어때요? 어떤 걸 먼저 해볼까요?"

사용자가 감정적일 때 (부정적 경험):
  → "힘든 경험이었지만, 그 속에서 배운 게 있다면 그게 가장 강력한 소재가 됩니다"
```

---

## 6. 기술 스택

| 영역 | 기술 | 이유 |
|------|------|------|
| 프론트엔드 | React 18 + Vite | insidejob과 동일 스택, 빠른 개발 |
| 스타일링 | Tailwind CSS | 유틸리티 우선, 반응형 쉬움 |
| 라우팅 | React Router v6 | SPA 라우팅 |
| 백엔드/DB | Supabase | Auth + PostgreSQL + Realtime |
| AI (현재) | Gemini Flash (무료) | 전 기능 통합, 비용 0원 |
| AI (To-be) | Haiku(대화) + Sonnet(자소서) | 하이브리드 — 자소서 품질 극대화 |
| AI 프록시 | Supabase Edge Function (`gemini-proxy`) | API 키 서버 보관, 인증·사용량 제한 집중 |
| 배포 | GitHub Pages | 무료 호스팅, GitHub Actions 자동 배포 |

---

## 7. DB 스키마 (핵심)

### experience_blocks
```sql
- id (uuid, PK)
- user_id (uuid, FK → auth.users)
- category (text) -- 팀프로젝트, 인턴, 공모전 등
- title (text) -- 블록 제목
- situation (text) -- 상황
- action (text) -- 행동
- result (text) -- 결과
- lesson (text) -- 배운점
- tags (text[]) -- #위기대응, #리더십 등 (10개 표준 역량으로 제약)
- recommended_questions (text[]) -- 추천 문항 유형
- strength_score (int) -- 경험 강도 1~5
- ai_insight (text) -- AI가 발견한 가치 포인트
- recommended_industries (jsonb) -- 업종/직무 AI 추천 캐시 { industries: [{name, reason}], roles: [] }
- created_at, updated_at
```

### interview_sessions
```sql
- id (uuid, PK)
- user_id (uuid, FK → auth.users)
- block_id (uuid, FK → experience_blocks, nullable)
- category (text)
- messages (jsonb) -- [{role, content, timestamp}]
- status (text) -- in_progress, completed, abandoned
- turn_count (int)
- created_at, completed_at
```

### usage_counters (2026-04-19 추가)
```sql
- id (uuid, PK)
- user_id (uuid, FK → auth.users)
- month (text) -- 'YYYY-MM'
- blocks_created (int) -- 월별 생성 블록 수
- cover_letters_generated (int) -- 월별 생성 자소서 수
- plan (text) -- 'free' | 'pro' | 'insight'
- UNIQUE (user_id, month)
```
- SELECT만 RLS 허용, 쓰기는 `increment_usage(user_id, month, column)` RPC만 (SECURITY DEFINER)
- Edge Function `gemini-proxy`에서 action이 `block_gen` | `answer_gen`일 때만 증가
- free 플랜이 한도 초과 시 402 `{ error: "limit_reached", scope, limit }` 반환

---

## 8. 구현 현황

### 완료 (2026-04-15)
- [x] 회원가입/로그인 (Supabase Auth)
- [x] AI 대화형 인터뷰 (5턴 고정, Gemini Flash)
- [x] 경험 블록 생성/저장/삭제
- [x] 대시보드 (블록 목록 + 역량 커버리지 + 집계 업종/직무 추천)
- [x] 블록 카드 펼침 시 인터뷰 대화 이력 보기 (interview_sessions 조회)
- [x] 블록 결과 화면 (업종/직무 AI 추천 + 역량 현황)
- [x] 업종/직무 추천 DB 캐싱 (recommended_industries jsonb) — AI 재호출 방지
- [x] 역량 태그 10개 표준 제약 + alias 매핑으로 커버리지 정확도 개선
- [x] 자소서 작성 (공고 파싱 → 블록 매칭 → 문항별 초안 생성)
- [x] 공고에 문항 없을 때 수동 입력/기본 문항 선택 지원
- [x] GitHub Pages 자동 배포

### 완료 (2026-04-26) — UI/UX + 자소서 워크플로 스프린트 (IMPROVEMENT_PLAN.md)
- [x] **P1 디자인 토큰 정비** — surface/shadow 토큰, 카테고리 그룹 컬러(학업/실무/도전/대외활동), Card 컴포넌트, lucide-react 아이콘 도입, SVG 별점
- [x] **P2 자소서 저장 인프라** — `cover_letters` + `cover_letter_answers` 테이블, 회사별 이력 페이지(`/cover-letter`), 작업 자동 저장(디바운스), 새로고침 시 이어서 편집
- [x] **P3 재생성/톤·강조/블록 수동 매칭** — 톤 4종/강조 3종 select, 다시 생성 버튼, 블록 변경 드롭다운, 답변 버전 누적(`is_active` 플래그), Edge Function 정책 변경(문항당 3회 무료 재생성)
- [x] **P4 대시보드 정보 위계 개편** — Hero zone(역량 레이더 + 다음 카테고리 추천 + 사용량), Notion 패널(좌 검색·필터·리스트 / 우 풀뷰), 업종 추천 collapsible 강등
- [x] **P5 랜딩 데모/시각화** — 자동 재생 인터뷰→블록 변환 데모, Before/After 비교 카드, 소셜 프루프(`public_block_count` RPC)
- [x] **P6 보강** — 인터뷰 중단 버튼(draft 보존), 업종 추천 백그라운드 lazy 재시도, route-level lazy 로드(첫 페인트 787KB → 204KB)

### 완료 (2026-04-19) — 보안/UX 스프린트
- [x] **Gemini API 키 서버 이동** — Supabase Edge Function `gemini-proxy` 도입, 클라이언트 번들에서 키 제거
- [x] **사용량 제한 인프라** — `usage_counters` 테이블 + `increment_usage` RPC (SECURITY DEFINER), 월별 집계
- [x] **무료 플랜 하드 한도** — 블록 3개/월, 자소서 1회/월 (Edge Function에서 402 반환)
- [x] **토스트 UI** — `ToastContext` 도입, `alert`/`console.error` 전부 교체
- [x] **자소서 병렬 생성** — `Promise.allSettled`로 문항별 동시 생성 (~4x)
- [x] **업종 추천 백그라운드화** — 블록 저장 시 동기 대기 제거, 저장 후 비동기 업데이트
- [x] **인터뷰 세션 복구** — localStorage 드래프트 + 재진입 시 "이어하기" 옵션
- [x] **블록 수정** — `/block/:id/edit` 라우트, STAR 필드/태그/강도/인사이트 편집
- [x] **SEO/OG 메타** — index.html description, og:*, twitter:card
- [x] **유료화 — 마감 패스 + Pro 구독** (2026-06) — value-first 게이팅, 토스 서버 승인, `passes`/`subscriptions`, 업그레이드 시트, 목 모드. 토스 키 발급만 남음.
- [x] **일일 유지보수 cron** (2026-06) — `cron-tasks`(구독 갱신 + 블록 백필) + GitHub Actions 스케줄 + Supabase 킵얼라이브

### To-do
- [ ] "블록" 네이밍 재검토 (예: 경험 카드)
- [ ] 회사 DB 구축 (업종/회사 정보 + 유저 블록 집계)
- [ ] 채용공고 자동 수집 (워크넷 공공 API) → "이 공고로 자소서 쓰기" 프리필
- [ ] 이메일 채널(Resend 등) → 주간 다이제스트 + 구독 갱신 D-2 알림
- [ ] AI 모델 하이브리드 (대화: Gemini Flash, 자소서: 상위 모델)
- [ ] 합격자 인사이트 (회사별 블록 통계, 문항별 경험 유형 분석)
- [ ] 소셜 로그인 (Google/Kakao)
- [ ] 자소서 PDF 내보내기
- [ ] insidejob 연동 (진단 결과 → 부족 역량 → 블록 빌더 유도)
- [ ] 면접 시뮬레이션 (블록 기반 예상 질문 + 모의 면접)
- [ ] 분석/이벤트 추적 (GA4 또는 PostHog) — 퍼널 측정
- [ ] TypeScript 전환 또는 PropTypes 도입
- [ ] 테스트 코드 (parseBlockFromResponse, mapTagToCompetency 등)

---

## 9. 과금 모델 (구현됨, 2026-06)

§3 과 동일 — 실제 적용된 플랜. 상세·정책은 [README — 결제/유료화](./README.md#결제--유료화).

| 구분 | 무료 | 마감 패스 (₩4,900, 단건) | Pro (₩9,900/월, 첫 달 ₩4,900) |
|------|------|--------------------------|-------------------------------|
| 블록 생성 | 월 3개 | — | 무제한 |
| 자소서 (공고당) | 첫 문항만 | 이 공고 전 문항 | 모든 공고 전 문항 |
| 문항당 다시쓰기 | 3회 | 3회 | 3회 |
| AI 모델 | Gemini Flash (서버 프록시, 모델/토큰 캡) | 〃 | 〃 |

- 단건은 구독으로 미는 앵커. 결제 시트 기본 강조 = Pro(첫 달 동가).
- **미구현**: 합격자 통계, PDF 내보내기, 갱신 D-2 알림(이메일 채널 필요).

### 비용 구조 (1사이클 = 블록 1개 + 자소서 4문항)
- Gemini Flash 전체: ~4원 (현재 단일 모델). 문항당 3회 상한 + 토큰 캡으로 상한 고정.
